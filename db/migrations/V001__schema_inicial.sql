-- ============================================================================
-- GestorIA - Esquema inicial de base de datos (MVP)
-- PostgreSQL 15+
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 6.1 Deuda viva por cliente
-- ----------------------------------------------------------------------------
CREATE VIEW v_deuda_por_cliente AS
SELECT
    c.id              AS cliente_id,
    c.nombre_fiscal,
    c.cif_nif,
    ...
    COALESCE(SUM(f.total), 0) - COALESCE(SUM(p.pagado), 0) AS deuda_pendiente
FROM clientes c
LEFT JOIN facturas f ON f.cliente_id = c.id
    AND f.estado NOT IN ('anulada', 'borrador')
LEFT JOIN LATERAL (
    SELECT COALESCE(SUM(pg.importe), 0) AS pagado
    FROM pagos pg
    WHERE pg.factura_id = f.id
) p ON TRUE
WHERE c.activo = TRUE
GROUP BY c.id, c.nombre_fiscal, c.cif_nif;

COMMENT ON VIEW v_deuda_por_cliente IS 'Resumen de facturación y deuda pendiente por cliente activo.';

-- ----------------------------------------------------------------------------
-- 6.2 Horas trabajadas por empleado y día
-- ----------------------------------------------------------------------------
CREATE VIEW v_horas_diarias AS
SELECT
    e.id           AS empleado_id,
    e.nombre,
    e.apellidos,
    DATE(fe.fecha_hora) AS fecha,
    fe.fecha_hora  AS hora_entrada,
    fs.fecha_hora  AS hora_salida,
    CASE
        WHEN fs.fecha_hora IS NOT NULL
        THEN EXTRACT(EPOCH FROM (fs.fecha_hora - fe.fecha_hora)) / 3600.0
        ELSE NULL
    END AS horas_trabajadas
FROM empleados e
JOIN fichajes fe ON fe.empleado_id = e.id AND fe.tipo_evento = 'entrada'
LEFT JOIN LATERAL (
    SELECT f2.fecha_hora
    FROM fichajes f2
    WHERE f2.empleado_id = e.id
      AND f2.tipo_evento = 'salida'
      AND f2.fecha_hora > fe.fecha_hora
    ORDER BY f2.fecha_hora ASC
    LIMIT 1
) fs ON TRUE;

COMMENT ON VIEW v_horas_diarias IS 'Pares entrada/salida con horas trabajadas por empleado y día.';

-- ----------------------------------------------------------------------------
-- 6.3 Resumen mensual operativo (para panel del gerente)
-- ----------------------------------------------------------------------------
...
SELECT
    ...
    -- Horas
    (SELECT COALESCE(SUM(
        EXTRACT(EPOCH FROM (fs.fecha_hora - fe.fecha_hora)) / 3600.0
    ), 0)
    FROM fichajes fe
    JOIN LATERAL (
        SELECT f2.fecha_hora
        FROM fichajes f2
        WHERE f2.empleado_id = fe.empleado_id
          AND f2.tipo_evento = 'salida'
          AND f2.fecha_hora > fe.fecha_hora
        ORDER BY f2.fecha_hora ASC
        LIMIT 1
    ) fs ON TRUE
    WHERE fe.tipo_evento = 'entrada'
      AND DATE_TRUNC('month', fe.fecha_hora) = DATE_TRUNC('month', periodo)
    ) AS total_horas,
    -- Trabajos activos
    (SELECT COUNT(*)
     FROM trabajos t
     WHERE t.estado IN ('pendiente', 'en_curso', 'bloqueado')
       AND DATE_TRUNC('month', t.created_at) <= DATE_TRUNC('month', periodo)
       AND (t.fecha_cierre IS NULL OR DATE_TRUNC('month', t.fecha_cierre) > DATE_TRUNC('month', periodo))
    ) AS trabajos_activos,
    -- Facturación emitida en el mes
    (SELECT COALESCE(SUM(f.total), 0)
     FROM facturas f
     WHERE f.estado NOT IN ('anulada', 'borrador')
       AND DATE_TRUNC('month', f.fecha_emision) = DATE_TRUNC('month', periodo)
    ) AS facturacion_mes,
    -- Deuda total pendiente
    (SELECT COALESCE(SUM(f.total), 0) - COALESCE(SUM(
        (SELECT COALESCE(SUM(pg.importe), 0) FROM pagos pg WHERE pg.factura_id = f.id)
    ), 0)
     FROM facturas f
     WHERE f.estado NOT IN ('anulada', 'borrador', 'pagada')
    ) AS deuda_total_pendiente
...
COMMENT ON VIEW v_resumen_mensual IS 'Resumen operativo mensual para el panel del gerente: horas, trabajos, facturación y deuda.';
