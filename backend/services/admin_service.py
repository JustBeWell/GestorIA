...
class AdminService:
        ...
        with db_connection() as connection:
            with connection.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if not registry.get("r1") or not registry.get("r2"):
                    return empty
                AdminService._ensure_ia_usage_schema(cursor)
                ...
                cursor.execute(
                    ...
                    WITH base AS (
                        SELECT
                            ...
                            COALESCE(nombre_usuario::text, 'desconocido') AS nombre_usuario,
                            COALESCE(NULLIF(mode, ''), 'respuesta') AS mode,
                            prompt_tokens,
                            completion_tokens,
                            total_tokens,
                            image_generations,
    ...
    @staticmethod
    def _ensure_ia_usage_schema(cursor) -> None:
